...
func createMovie(w http.ResponseWriter, r *http.Request){
	w.Header().Set("Content-Type","application/json")
	var movie Movie
	_ = json.NewDecoder(r.Body).Decode(&movie) // we get the value in the above movie variable

	movie.ID = strconv.Itoa(rand.Intn(100100))
...
func getMovie(w http.ResponseWriter, r * http.Request){
	...
	for _, item := range movies{
		if item.ID == params["id"]{
			json.NewEncoder(w).Encode(item) //returns the movie as json 
			return
		}
	}
}
